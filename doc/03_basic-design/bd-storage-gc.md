...
本設計書は、不要になった孤立（到達不能）オブジェクトをアトミックかつ安全に削除する Mark-and-Sweep GC、並行コミット中の誤削除を防ぐ Grace Period（猶予期間）保護、並行 Tri-Color Marking & SATB Write Barrier アルゴリズム、およびエポック置換型並行 Packfile Compaction の基本設計書である。

本書は `doc/specs/spec-storage.md` の第6節 (6.1), 第8節 (8.1), 第12節 (12.1, 12.2), 第18節 (18.1), 第19節 (19.1) および `doc/01_architecture/sfvcs-design.md` の第171.7, 171.14節の仕様を完全網羅し、カプセル化された Garbage Collector Service モジュールとして詳細を定義する。
...
# 3. Mark-and-Sweep 手順 & Grace Period（猶予期間）保護

### 3.1 Mark-and-Sweep シーケンスステップ
...
pub fn satb_write_barrier(obj_cid: &[u8; 32], gc_state: &AtomicGcState, grey_queue: &mut Vec<[u8; 32]>) {
...
# 5. エポック置換型並行 Lockless Packfile Compaction & In-flight Register

### 5.1 In-flight Operation Generation Register
現在進行中の並行コミット処理が参照している世代（Generation Number）をレジスタに記録し、GC 実行時にその世代のオブジェクトが削除されないようアトミックに保護する。
